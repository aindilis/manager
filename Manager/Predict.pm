...
sub Execute {
  ...
  $self->GenerateBins();
  # return
  my $result = $self->ComputeNextWeek
      (Params => $self->FitSleep());
  if ($args{Display}) {
    $self->DisplayParams;
  }
  $self->Plot();
...
sub RetrieveTimes {
  ...
  # $self->EarliestTime;
  $s = "select ID, UNIX_TIMESTAMP(Date) from events where Event regexp '(closed|conceal|opened|display)::' and UNIX_TIMESTAMP(Date) > ".
  $self->EarliestTime;
  $ret = $self->MyMySQL->Do
    (Statement => $s);
  my @times;
...
sub ComputeNextWeek {
  ...
  my @week;
  foreach my $d (0..28) {
    my $bedtime = $params->{BedTimeEpoch} + 3600 * ($d * $params->{Spacing});
    my $awaketime = $params->{BedTimeEpoch} + 3600 * ($d * $params->{Spacing} + $params->{SleepWidth});
    push @week, [[$bedtime,$self->GetTimeFromEpoch(Epoch => $bedtime)],
		 [$awaketime,$self->GetTimeFromEpoch(Epoch => $awaketime)]];
  }
  return \@week;
}
